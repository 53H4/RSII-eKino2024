...
class _ReservationDetailsScreenState extends State<ReservationDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.reservation != null ? 'Reservation Details' : 'New Reservation'),
        leading: IconButton(
        ...
        children: [
          ...
          FormBuilderTextField(
            ...
            initialValue: _projectionsList
                        ?.firstWhere(
                            (projection) => projection.projectionId.toString() == _initialValue['projectionId'])
                        ?.dateOfProjection !=
                    null
                ? DateFormat('dd.MM.yyyy HH:mm').format(_projectionsList!
                    .firstWhere((projection) => projection.projectionId.toString() == _initialValue['projectionId'])
                    .dateOfProjection!)
                : 'Unknown',
            enabled: false,
          ),
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              //         reservation.projectionId == int.parse(_initialValue['projectionId'])) ??
              //     false;

              // final isSelected =
              //     _initialValue['row']?.split(',').contains(seatValue);
              final isReserved = false;
              ...
              return GestureDetector(
                onTap: () {
                  // setState(() {
                  //   if (!isReserved) {
                  //     final selectedSeats = _initialValue['row']?.split(',') ?? [];
                  //     if (isSelected!) {
                  //       selectedSeats.remove(seatValue);
                  //     } else {
                  //       selectedSeats.add(seatValue);
                  //     }
                  //     _formKey.currentState?.fields['row']?.didChange(selectedSeats.join(','));
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: (isReserved ? Colors.red : (isSelected ? Colors.blue : Colors.grey)),
                    border: Border.all(color: Colors.grey),
...
class ScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Path path = Path()
      ...
      ..quadraticBezierTo(controlPoint1.dx, controlPoint1.dy, size.width / 2, size.height * 0.25)
      ..quadraticBezierTo(controlPoint2.dx, controlPoint2.dy, size.width, size.height);
